...
exports.LoginDispatch = async (req, res) => {
  const Email = req.body.Email;
  const password = req.body.password;
  let loadedUser;
  const user = await Dispatch.findOne({ Email: Email });
  if (!user) {
  ...
  if (ismatch) {
    ...
    console.log("JSON WEB-TOKEN OF Dispatcher is :");
...
exports.sendEmail = async (req, res) => {
  const Email = req.body.Email;
  console.log(Email);
  let data = await Dispatch.findOne({ Email: Email });
  if (data) {
...
exports.changePassword = async (req, res) => {
  ...
  if (data) {
    ...
    if (diff < 0) {
      ...
      let user = await Dispatch.findOne({ Email: req.body.Email });
